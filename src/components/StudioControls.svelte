<script lang="ts">
  ...
  // ---- derived: options / active template / field helpers ----
  ...
  function labelFor(path: string) {
    ...
    return (
      tplOptions.find((t) => t.path === path)?.name ?? "— choose a template —"
    );
  }
...
<div
  class="w-full lg:min-w-[454px] border border-neutral-800 bg-neutral-900/60 rounded-2xl p-4 text-neutral-100 space-y-4"
>
  <!-- Aspect -->
  <div>
    <span
      class="cursor-default block text-xs font-semibold tracking-wide uppercase opacity-70 mb-1"
      >Aspect</span
    >
        ...
        >
          {#each tplOptions as t, i}
            <button
              ...
              tabindex="-1"
              data-index={i}
              class="w-full px-3 py-2 cursor-pointer flex items-center justify-between
  ...
  {#if activeTpl}
    ...
    {#if has("bg")}
      <div>
        ...
        <div
          ...
          ondrop={onDzDrop}
          onpaste={onDzPaste}
          class="cursor-pointer relative w-full rounded-xl border-2 border-dashed p-4 transition
                 bg-neutral-900/60 border-neutral-700 hover:border-neutral-500
        ...
        >
          ...
          <div class="flex items-center gap-3">
            <div
              class="shrink-0 rounded-lg bg-neutral-800 px-3 py-2 text-sm text-neutral-100"
            >
              Choose file
            </div>
            <div class="min-w-0 flex-1">
              ...
              {:else}
                <div class="text-sm text-neutral-400">
                  Click to browse, or <span class="text-neutral-200"
                    >drop / paste</span
                  > an image
                </div>
    ...
    {#if has("credit")}
      <div>
        <label>
          <span
            class="block text-xs font-semibold tracking-wide uppercase opacity-70 mb-1"
          >
  ...
  <div class="flex items-center justify-between">
    <label class="flex items-center gap-3 cursor-pointer">
      <div class="relative">
        <input type="checkbox" class="sr-only peer" bind:checked={generate2x} />
        <div
          class="w-11 h-6 bg-neutral-700 rounded-full peer-checked:bg-[#50C2BE] transition-colors"
        ></div>
        <div
          class="absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform peer-checked:translate-x-5"
        ></div>
      </div>
      <span class="text-sm font-mono">{generate2x ? "2K" : "HD"}</span>
    </label>
